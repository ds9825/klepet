...
$(document).ready(function() {
  ...
  socket.on('dregljaj', function(rezultat) {
    var dreg = socket.socket.sessionid;
    if (dreg != rezultat.socket) {
    $('#vsebina').jrumble();
  ...
  socket.on('sporocilo', function (sporocilo) {
    ...
    pokaziSliko(sporocilo.besedilo);
  ...
  socket.on('uporabniki', function(uporabniki) {
    ...
    for (var i=0; i < uporabniki.length; i++) {
      ...
      $('#seznam-uporabnikov div').click(function() {
        ...
        $('#poslji-sporocilo').val( '/zasebno "' + prejemnik + '"' );
        $('#poslji-sporocilo').focus();
      });
  });
...
function pokaziSliko(slika) {
  ...
  for (var a in t) {
    ...
    if (regex.test(t[a])) {
      $('#sporocila').append(divElementSlika(t[a]));
    }
  }
}
